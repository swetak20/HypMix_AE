...
def experiment_name_non_mnist(dataset='latent_cifar10',
                    arch='',
                    epochs=400,
                    dropout=True,
                    batch_size=64,
...
def train(train_loader, model, optimizer, epoch, args, log):
    ...
    for i, (input) in enumerate(train_loader):
        ...
        if args.train == 'mixup':
            ...
            output = model(input_var, mixup= True, mixup_alpha = args.mixup_alpha)
            ...
            input_var= Variable(input)
            output = model(input_var, mixup_hidden= True, mixup_alpha = args.mixup_alpha)
            # loss = bce_loss(softmax(output))#mixup_criterion(target_a, target_b, lam)
            """
